"""
PyTorch training entrypoint for PI0/PI05/CSPi0/PostBC with multi-GPU and multi-node (DDP) support.
This script mirrors the behavior of the JAX trainer (`scripts/train.py`) but runs
entirely in PyTorch using the PyTorch model implementations and the existing config/data
pipeline from `src/tmrl_openpi/training/config.py` and `src/tmrl_openpi/training/data_loader.py`.
...
Multi-GPU (single node):
  ...
  Example:
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_pytorch.py cspi0_libero --exp_name pytorch_ddp_test
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_pytorch.py cspi0_libero --exp_name pytorch_ddp_test --resume
Multi-Node Training:
	torchrun \
...
def train_loop(config: _config.TrainConfig):
    ...
    # Build model
    if not isinstance(config.model, tmrl_openpi.models.pi0_config.Pi0Config):
        model_cfg = tmrl_openpi.models.pi0_config.Pi0Config(
    ...
    # Load weights from pytorch_weight_path if specified (for fine-tuning)
    ...
    # so we implement the freeze directly here via isinstance check.
